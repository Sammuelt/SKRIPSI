...
<div class="flex flex-col justify-center">
    ...
    <form method="POST" action="{{ route('store.akun') }}">
        ...
        <div class="mb-6">
            <label for="id_user" class="block mb-2 text-sm font-medium text-gray-900">Pilih Guru</label>
            <select class="form-control w-full text-gray-900 bg-gray-50 border border-gray-300 focus:ring-4 focus:outline-none focus:ring-blue-500 font-medium rounded-lg text-sm px-5 py-2.5 text-center inline-flex items-left justify-between" id="guru" name="guru_id">
                <option value="">--PILIH GURU--</option>
                @foreach($users as $user)
                    <option value="{{ $user->id }}" data-email="{{ $user->email }}">{{ strtoupper($user->nama) }}</option>
                @endforeach
            </select>
        </div>

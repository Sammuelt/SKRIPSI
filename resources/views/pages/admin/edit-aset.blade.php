    ...
    <div class="flex flex-col justify-center">
        ...
        <form action="{{ route('update.inventaris', $aset->id) }}" method="POST">
            ...
            <div class="grid gap-6 mb-6 md:grid-cols-2">
                <div>
                    <label for="dana_pembelian" class="block mb-2 text-sm font-medium text-gray-900">Dana Digunakan</label>
                    <input type="text" name="dana_pembelian" id="dana_pembelian" value="{{ $aset->dana_pembelian }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required />
                </div>
                <div>
                    <label for="kode_barang" class="block mb-2 text-sm font-medium text-gray-900">Kode Barang</label>
                    <input type="text" id="kode_barang" name="kode_barang" value="{{ $aset->kode_barang }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required />
                </div>
                <div>
                    <label for="nama_barang" class="block mb-2 text-sm font-medium text-gray-900">Nama Barang</label>
                    <input type="text" id="nama_barang" name="nama_barang" value="{{ $aset->nama_barang }}"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5"
                        required />
                </div>
                <div>
                    ...
                    <select id="kondisi" name="kondisi"
                        ...
                        <option value="sangat_bagus" {{ $aset->kondisi == 'sangat_bagus' ? 'selected' : '' }}>Sangat Bagus</option>
                        <option value="bagus" {{ $aset->kondisi == 'bagus' ? 'selected' : '' }}>Bagus</option>
                        <option value="cukup bagus" {{ $aset->kondisi == 'cukup bagus' ? 'selected' : '' }}>Cukup Bagus</option>
                        <option value="tidak_bagus" {{ $aset->kondisi == 'tidak_bagus' ? 'selected' : '' }}>Tidak Bagus</option>
